...
exp_outcome_naive_men <- new_expected_outcome(test,response,control,treatment_list,naive_predictions$Treatment)
exp_inc_outcome_naive_men <- new_expected_quantile_response(test,response,control,treatment_list,naive_predictions)

naive_predictions <- data.frame(cbind(rep("women_treatment",nrow(test)),sample(nrow(test),replace = T)))
colnames(naive_predictions) <- c("Treatment","random_uplift")
exp_outcome_naive_women <- new_expected_outcome(test,response,control,treatment_list,naive_predictions$Treatment)
exp_inc_outcome_naive_women <- new_expected_quantile_response(test,response,control,treatment_list,naive_predictions)
```
...
### Model Comparison
```{r}
temp_df = data.frame(models=c("C_Forest","C_Tree","Rzp_Tree","Rzp_Forest","Simple_Tree","Simple_Forest","Naive Men", "Naive Women"),values = c(exp_outcome_c_forest,exp_outcome_c_tree,exp_outcome_rzp,exp_outcome_rzp_forest,exp_outcome_simple,exp_outcome_simple_forest,exp_outcome_naive_men,exp_outcome_naive_women))
...
name_vec <- c(rep("C_Forest",11),rep("C_Tree",11),rep("Rzp_Tree",11),rep("Rzp_Forest",11),rep("Simple_Tree",11),rep("Simple_Forest",11),rep("Naive Men",11),rep("Naive Women",11))
temp_results <- cbind(c(exp_inc_outcome_c_forest,exp_inc_outcome_c_tree,exp_inc_outcome_rzp,exp_inc_outcome_rzp_forest,exp_inc_outcome_simple,exp_inc_outcome_simple_forest,exp_inc_outcome_naive_men,exp_inc_outcome_naive_women))
