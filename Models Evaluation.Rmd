...
### Model Comparison
```{r}
temp_df <- cbind(exp_outcome_c_forest,exp_outcome_c_tree,exp_outcome_rzp,exp_outcome_rzp_forest,exp_outcome_simple)
colnames(temp_df) <- c("C_Forest","C_Tree","Rzp_Tree","Rzp_Forest","Simple_Tree")
barplot(temp_df)

temp_vec <- rep(seq(0,1,0.1),5)
name_vec <- c(rep("C_Forest",11),rep("C_Tree",11),rep("Rzp_Tree",11),rep("Rzp_Forest",11),rep("Simple_Tree",11))
temp_results <- cbind(c(exp_inc_outcome_c_forest,exp_inc_outcome_c_tree,exp_inc_outcome_rzp,exp_inc_outcome_rzp_forest,exp_inc_outcome_simple))
...
ggplot(data = temp_df, aes(x="Percentile", y="Expected_Outcome")) + geom_line(aes(colour="Model"))
