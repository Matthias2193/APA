...
## TODO
# rerun spend models on real data split -> same idx ??
# run Causal Tree on Spend
# Interpret resulst
# start graphs from 0
...
raw_tree <- create_node(train,0,100,treatment_list,response,'control',test_list,normalize = TRUE)

# Partition training data for pruning

# takes a lot of time..
#conversion
pruned_tree <- prune_tree(raw_tree,val,train,target = response) #conversion
...
raw_tree <- create_node(train,0,100,treatment_list,response,'control',test_list,normalize = TRUE,criterion = 2)
...
# Expected Response per targeted customers
rzp_tree_exp_conv_1 <- expected_percentile_response(pred)

rzp_tree_mat_conv_1 <- matching_evaluation(pred, 'control')
```
...
colnames(rzp_tree_mat_conv)[2] <- colnames(c_tree_mat_conv)[2] <- "Mens E-Mail"
colnames(rzp_tree_mat_conv)[3] <- colnames(c_tree_mat_conv)[3] <- "Womens E-Mail"

## Display of results only for SMA models
melt(rbind(sma_logit_conv_mat[, c("Percentile","Model","Mens E-Mail")], sma_dt_conv_mat[, c("Percentile","Model","Mens E-Mail")], sma_rf_conv_mat[, c("Percentile","Model","Mens E-Mail")], rzp_tree_mat_conv[, c("Percentile","Model","Mens E-Mail")], c_tree_mat_conv[, c("Percentile","Model","Mens E-Mail")]), id.vars = c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
...
melt(rbind(sma_logit_conv_mat[, c("Percentile","Model","Womens E-Mail")], sma_dt_conv_mat[, c("Percentile","Model","Womens E-Mail")], sma_rf_conv_mat[, c("Percentile","Model","Womens E-Mail")], rzp_tree_mat_conv[, c("Percentile","Model","Womens E-Mail")], c_tree_mat_conv[, c("Percentile","Model","Womens E-Mail")]), id.vars = c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
  geom_line(aes(y = value, group= Model, color= Model) ) +
  labs(
    color="Base Learner",
...
```{r, eval=FALSE}
rzp_tree_exp_conv$Model<- rzp_tree_mat_conv$Model <- "Rzp Tree"
rzp_tree_exp_conv_1$Model<- rzp_tree_mat_conv_1$Model <- "Rzp Tree Simple"


melt(rbind(rzp_tree_mat_conv[, c("Percentile","Model","max T")], rzp_tree_mat_conv_1[, c("Percentile","Model","max T")]), id.vars = c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
  geom_line(aes(y = value, group= Model, color= Model) ) +
  labs(
    color="Base Learner",
...
```{r}
# Expected Outcome
melt(rbind(rzp_tree_exp_conv_1[, c("Percentile","Model","Expected Outcome")], rzp_tree_exp_conv[, c("Percentile","Model","Expected Outcome")]), id.vars = c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
  geom_line(aes(y = value, group= Model, color= Model) ) +
  labs(
    color="Model",
...
colnames(c_tree_mat_spend)[2] <- colnames(rzp_tree_mat_spend)[2] <- "Mens E-Mail"
colnames(c_tree_mat_spend)[3] <- colnames(rzp_tree_mat_spend)[3] <- "Womens E-Mail"

## Display of results only for SMA models
melt(rbind(sma_dt_mat_spend[, c("Percentile","Model","Mens E-Mail")], sma_rf_mat_spend[, c("Percentile","Model","Mens E-Mail")], rzp_tree_mat_spend[, c("Percentile","Model","Mens E-Mail")] , c_tree_mat_spend[, c("Percentile","Model","Mens E-Mail")]), id.vars  =c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
...
melt(rbind(sma_dt_mat_spend[, c("Percentile","Model","Womens E-Mail")], sma_rf_mat_spend[, c("Percentile","Model","Womens E-Mail")], rzp_tree_mat_spend[, c("Percentile","Model","Womens E-Mail")], c_tree_mat_spend[, c("Percentile","Model","Womens E-Mail")][, c("Percentile","Model","Womens E-Mail")]), id.vars = c("Percentile","Model")) %>% ggplot(aes(x = Percentile)) +
  geom_line(aes(y = value, group= Model, color= Model) ) +
  labs(
    color="Base Learner",
    title = "All Models - Women's Treatment Curve",
