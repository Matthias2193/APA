...
raw_tree <- create_node(train,0,100,treatment_list,response,control,test_list,normalize = F)
...
rzp_forest_mat_conv <- matching_evaluation(pred, "control")
```
...
colnames(random_conv_mat)[2] <- colnames(rzp_tree_mat_conv)[2] <- colnames(rzp_tree_mat_conv_simple)[2] <- colnames(rzp_forest_mat_conv)[2] <- colnames(c_tree_mat_conv)[2] <- colnames(c_forest_mat_conv)[2] <- "Mens E-Mail"
colnames(random_conv_mat)[3] <-colnames(rzp_tree_mat_conv)[3] <-colnames(rzp_tree_mat_conv_simple)[3] <-colnames(rzp_forest_mat_conv)[3] <- colnames(c_tree_mat_conv)[3] <- colnames(c_forest_mat_conv)[3] <- "Womens E-Mail"
...
rzp_forest_mat_spend <- matching_evaluation(pred, "control")
```
