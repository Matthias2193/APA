  ...
  if (length(test_list$categorical) > 0) {
    for (x in 1:length(test_list$categorical)) {
      ...
      for (y in 1:length(test_list$categorical[[x]])) {
        ...
        if (criterion == "simple") {
          gain_list <- c(gain_list, simple_gain(
            t, treatment, control, target, temp_data, "categorical", temp_name,
            min_split, parent_predictions
          ))
        } else if (criterion == "max") {
          gain_list <- c(gain_list, max_gain(
            t, treatment, control, target, temp_data, "categorical", temp_name,
            min_split, parent_predictions
          ))
        } else {
          gain_list <- c(gain_list, frac_gain(
            t, treatment, control, target, temp_data, "categorical", temp_name,
            min_split, parent_predictions
          ))
        }
  ...
  if (length(test_list$numerical) > 0) {
    for (x in 1:length(test_list$numerical)) {
      ...
      for (y in 1:length(test_list$numerical[[x]])) {
        ...
        if (criterion == "simple") {
          gain_list <- c(gain_list, simple_gain(
            t, treatment, control, target, temp_data, "numerical", temp_name,
            min_split, parent_predictions
          ))
        } else if (criterion == "max") {
          gain_list <- c(gain_list, max_gain(
...
dom_train_tree <- function(data, depth, max_depth, treatment_list, target, control, random = FALSE,
  ...
  temp_split <- select_split(
    test_list = test_list, treatment = treatment_list, control, target, data, criterion,
...
assign_val_predictions <- function(tree, val_data, treatment_list, test_list, target, control) {
  ...
  if (tree[["type"]] != "leaf") {
    if (nrow(val_data) == 0) {
      ...
      tree[["right"]] <- assign_val_predictions(
        tree[["right"]], val_data, treatment_list, test_list, target,
        control
      )
    } else {
      ...
      tree[["right"]] <- assign_val_predictions(
        tree[["right"]], data_right, treatment_list, test_list, target,
        control
      )
    }
  }
